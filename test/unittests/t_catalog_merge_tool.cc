...
TEST_F(T_CatalogMergeTool, CRUDwithCache) {
  ...
  CatalogTestTool tester(repo_name, "http://127.0.0.1:8083");
  EXPECT_TRUE(tester.Init());

  std::cerr << "test tool " << tester.repo_name() << std::endl;
  MockProxyServer proxy_server(8083);
  MockFileServer file_server(8082, tester.repo_name() + "/data");

  std::cerr << tester.download_manager()->GetProxyList() << std::endl;
  ...
  EXPECT_TRUE(spec3.AddDirectory("my_extra_dir", "dir", 4096));
  EXPECT_TRUE(spec3.AddFile("extra.txt", "dir/my_extra_dir", hashes[4], 4024));

  EXPECT_TRUE(tester.Apply("third", spec3));


  UniquePtr<ServerTool> server_tool(new ServerTool());
  EXPECT_TRUE(server_tool->InitDownloadManager(true, "http://127.0.0.1:8083/"));

  server_tool->download_manager()->SetProxyChain("http://127.0.0.1:8083", "",
                                   download::DownloadManager::kSetProxyRegular);
  ...
  LogCvmfs(kLogCvmfs, kLogStdout, "Local cache dir %s", local_cache_dir.c_str());
  ...
  perf::Statistics statistics2;
  receiver::CatalogMergeTool<catalog::WritableCatalogManager,
                            catalog::SimpleCatalogManager>
    merge_tool2(params.stratum0, history[2].second,
                history[3].second,
                PathString(""), GetCurrentWorkingDirectory() + "/merge_tool",
                server_tool->download_manager(), &first_manifest, &statistics2,
                local_cache_dir);
  EXPECT_TRUE(merge_tool2.Init());

  EXPECT_TRUE(merge_tool2.Run(params, &output_manifest_path, &final_rev));
  EXPECT_EQ(3U, final_rev);

  LogCvmfs(kLogCvmfs, kLogStdout, "Proxy server calls %d", proxy_server.num_processed_requests());
  LogCvmfs(kLogCvmfs, kLogStdout, "File server calls %d", file_server.num_processed_requests());

  std::vector<std::string> dirs = FindDirectories(local_cache_dir);

  std::vector<std::string> txn_files = FindFilesByPrefix(local_cache_dir + "/txn", "");

  for (size_t i = 0; i < txn_files.size(); i++) {
    std::cerr << txn_files[i] << std::endl;
  }
  
  
  for (size_t i = 0; i < dirs.size(); i++) {
    std::vector<std::string> files = FindFilesByPrefix(dirs[i], "");

    for (size_t j = 0; j < files.size(); j++) {
      if (files[j].at(files[j].size() -1) != '.') {
